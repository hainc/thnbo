...
namespace THNBO\Grafika\Gd;

use THNBO\Grafika\DrawingObjectInterface;
use THNBO\Grafika\EditorInterface;
use THNBO\Grafika\FilterInterface;
use THNBO\Grafika\Gd\Helper\GifHelper;
use THNBO\Grafika\Gd\ImageHash\DifferenceHash;
use THNBO\Grafika\Grafika;
use THNBO\Grafika\ImageInterface;
use THNBO\Grafika\ImageType;
use THNBO\Grafika\Color;
use THNBO\Grafika\Position;
...
final class Editor implements EditorInterface
{
     ...
     * @throws \Exception
     */
    public function crop( &$image, $cropWidth, $cropHeight, $position, $offsetX = 0, $offsetY = 0)
    {
    ...
    public function resize(&$image, $newWidth, $newHeight, $mode = 'fit')
    {
        ...
        switch ($mode) {
            case 'exact':
                ...
                break;
            case 'fill':
                $this->resizeFill($image, $newWidth, $newHeight, $position);
                break;
            case 'exactWidth':
     ...
     * @return Editor
     */
    public function resizeFill(&$image, $newWidth, $newHeight, $position)
    {
        $width  = $image->getWidth();
        $height = $image->getHeight();
        $ratio  = $width / $height;
        ...
        $this->crop($image, $newWidth, $newHeight, $position); // Trim excess parts
